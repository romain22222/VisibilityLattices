...
points. Let $p \in K$. Then $\pi^{-1}_j( \pi_j(p)) \cap \Star{K}$ is a set of cells having
...
Figure~\ref{fig:lattice-representation} for an illustration).\comment[id=JO]{Mettre à jour la figure ``right'' !}
...
shapes (Figure~\ref{fig:meanvisibility-gridstep}). It follows a law
proportional to $1/\sqrt{h}$, so a the average continuous visibility
distance is proportional to $\sqrt{h}$. As shown in the next section,
for real 3d images, a maximal radius below 10 is sufficient for normal
and curvature estimation.\comment[id=JO]{Update paragraph starting
here} Finally, we have measured the same average visibility distance
on 3 piecewise smooth shapes with different $\Starop_1$
(Figure~\ref{fig:meanvisibility-gridstep-piecewise}).  We can observe
several regimes: for polyhedra, the average visibility is
proportionnal to $h^{-3/2}$ whatever $\Starop$ we use; for other
shapes, the average visibility is proportionnal to $h^{-1}$ for
$\Starop_1$ but to $h^{-3/2}$ for $\Starop_2$. Above a certain
gridstep, we think the visibility is limited by the maximal visibility
size we use to compute it (the limitation is computed as proportional
to $h^{-1/2}$).
...
\begin{figure}
    ...
    \caption{\label{fig:meanvisibility-gridstep-piecewise} Mean discrete distance of visibility as a function of the gridstep $h$ for 3 piecewise smooth shapes (see text).}
\end{figure}
