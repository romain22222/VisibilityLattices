...
Most of the shapes follows a law in $\Theta\left( h^{-\frac{1}{2}} \right)$,
excepted rcube and leopold, which contains flat surfaces which makes the
visibility distance grow faster than the previous law (measured to be between
$\Theta\left( h^{-\frac{3}{5}} \right)$ and $\Theta\left( h^{-\frac{2}{3}} \right)$).

We also tried this measurement on an Ellipsoid Union with $\Starop_1$ and $\Starop_2$
...
\begin{figure}
    ...
    \caption{\label{fig:meanvisibility-gridstep-piecewise} Mean discrete distance of visibility as a function of the gridstep $h$ for the piecewise smooth shape Ellipsoid Union (see text).}
\end{figure}
