...
Figure~\ref{fig:errors-normals-piecewise} shows the same errors computed on 2 piecewise smooth shapes
(cubesphere and double ellipsoid).
In order to capture the behavior of the normal estimators,
we ignore the points that are closer than $2h$ to the singularities of the shapes (see~\ref{fig:singularities-showcase}).
We see that the RMSE and $E_{\max}$ are much better at higher gridsteps than both II and PCA, with a convergence rate of $h^{\frac{1}{2}}$,
II and PCA catching back on the error at lower gridsteps is due to to the singularity zone being
at a continuous width instead of a discrete width, which allows them to be correct at a certain point. It however doesn't seem
...
\comment[id=ROM]{I need to redo the normal errors for VNs as they do not correspond anymore to fig:errors-normals-showcase-ellipsoids}
