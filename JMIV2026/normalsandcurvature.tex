...
\begin{figure*}
    ...
    The error is colored from white (0) to red (0.1). II normals are computed using a radius of
    $r=4.5$, PCA with a discrete radius of 4 (empirical), while our
    normal estimator are computed using a deviation of $\sigma=5h^{-\frac{1}{2}}$, with a star of 1 and with a weighted kernel.}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[
                    ...
                    domain=0.12:0.4,
                    samples=200,
                ] {0.02*x^(1.66)} node[pos=0.4, anchor=north east, yshift=10pt] {$h^{\frac{5}{3}}$};
            \end{axis}
        \end{tikzpicture} \\
        \begin{tikzpicture}
            \begin{axis}
...
%                ymin=0.01, ymax=0.3,
                ...
                \addplot[
                    ...
                    domain=0.12:0.4,
                    samples=200,
                ] {0.4*x^(1.5)} node[pos=0.2, anchor=south west] {$h^{\frac{3}{2}}$};
            \end{axis}
        \end{tikzpicture} \\
        \begin{tikzpicture}
            \begin{axis}[
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addlegendentry{II}
            \addlegendimage{mark=asterisk, only marks}
            \addlegendentry{PCA}
