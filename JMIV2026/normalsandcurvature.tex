...
\replaced[id=JO]{We propose a new normal estimator on digital surfaces,
  ...
  approximates the geometry near sharp or salient features.}{We
  propose a new normal estimator on digital surfaces, which uses the
  visibility of the point of interest while taking into account a
  user-given scale $\sigma$. If $\sigma$ is proportionnal to the
  average distance between visible points (so some
  $\Theta\left(\sqrt{h}\right)$) We propose a new normal estimator on
  digital surfaces, which uses the visibility of the point of interest
  while taking into account a user-given scale $\sigma$. If $\sigma$
  is proportionnal to the average distance between visible points (so
  some $\Theta\left(\sqrt{h}\right)$), then this estimator is observed
  to be multigrid convergent along digitization of smooth
  shapes. However, since the computation window is limited by the
  visibility, it better approximates the geometry near sharp or
  salient features.}
...
any positive function. \added[id=JO]{We set $\sigma := \frac{1}{\#
...
\newcommand{\formatImageFigNormalesScale}[1]{%
    \includegraphics[width=0.33\columnwidth]{./pictures/figNormalesScale/#1}%
}
...
\begin{figure*}
    ...
    \begin{tabular}{|c||c|c|c|c|}
        ...
        \hline
        \multicolumn{5}{c}{\includegraphics{./pictures/figNormalesScale/legend}} \\
    \end{tabular}
    \caption{Normal error on the ellipsoids shape for different methods and different gridsteps.
    VN stands for Visibility Normal, and the subscript stands for the kernel used (Uniform, Gaussian or Ring).
    The error is colored from white (0) to deep red (0.1). II normals are computed using a radius of
    $r=4.5$, PCA with a discrete radius of 4 (empirical), while our
    normal estimator are computed using a deviation of $\sigma=5h^{-\frac{1}{2}}$}
...
$E_{\max}$. The ring kernel seems to be slightly worse than the Gaussian kernel,
...
Figure~\ref{fig:errors-normals-piecewise} shows the same errors computed on 3 piecewise smooth shapes
(tetrahedron, cubesphere and double ellipsoid). \comment[id=JO]{Update paragraph with two shapes only} In order to capture the behavior of the normal estimators,
we ignore the points that are closer than $2h$ to the singularities of the shapes (see~\ref{fig:singularities-showcase}).
We see that the RMSE and $E_{\max}$ are much better than both II and PCA, with a convergence rate of $h^3$,
while both II and PCA stop converging after a certain threshold (here around h=0.2). It however doesn't seem
to have a better kernel between the ring and the gaussian kernel, sometimes one is better than the other, but
it really depends on what kind of shapes we are facing with. We can also notice that the singularity zone on the ellipsoids
...
%                ymin=0.01, ymax=0.3,
                ...
                \addplot[
                    ...
                    domain=0.12:0.4,
                    samples=200,
                ] {0.4*x^(0.5)} node[pos=0.2, anchor=south west] {$h^{\frac{1}{2}}$};
            \end{axis}
        \end{tikzpicture} \\
        \begin{tikzpicture}
            \begin{axis}[
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        [
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/L2/tetrahedron-II.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/L2/tetrahedron-PCA.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/L2/tetrahedron-VNG.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/L2/tetrahedron-VNR.txt};
            ...
            \addplot[
                ...
                domain=0.12:0.4,
                samples=200,
            ] {0.6*x^(3)} node[pos=0.5, anchor=north east] {$h^{3}$};
        \end{axis}
    \end{tikzpicture}
    \begin{tikzpicture}
        \begin{axis}
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/Loo/tetrahedron-II.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/Loo/tetrahedron-PCA.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/Loo/tetrahedron-VNG.txt};
            ...
            y=error,
            col sep=comma
            ] {./graphDataNormals/piecewise/Loo/tetrahedron-VNR.txt};
            ...
            \addplot[
                ...
                domain=0.12:0.4,
                samples=200,
            ] {5*x^(3)} node[pos=0.2, anchor=south west] {$h^{3}$};
        \end{axis}
    \end{tikzpicture}
    \begin{tikzpicture}
        \begin{axis}[
...
maximal visibility distance $2\sigma$.\comment[id=JO]{Text to update} These parameters were
