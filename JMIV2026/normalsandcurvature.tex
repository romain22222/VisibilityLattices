...
In practice, we chose to test 3 different kernels. First, the Uniform kernel ($\Kernel{x} = \mathbbm{1}_{x \leq 2\sigma}$)
aims at weighting everything the same, whatever its distance to p. The Gaussian kernel
($\Kernel{x} = e^{-\frac{x^2}{2\sigma^2}}$) aims at weighting closer data more than further ones, arguing closer data
represents better the information than further one. Finally, the Ring kernel
($\Kernel{x} = \frac{x}{2\sigma^2}e^{-\frac{x^2}{2\sigma^2}}$) aims at weighting data in middle range more
than the rest, trying to extrapolate the tangential information of the visibility instead of
its proximity. In~\ref{fig:errors-normals-showcase-ellipsoids}, we see that the Uniform kernel makes a lot more errors than the other two,
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[
                    ...
                    domain=0.12:0.4,
                    samples=200,
                ] {0.02*x^(0.66)} node[pos=0.4, anchor=north east, yshift=10pt] {$h^{\frac{2}{3}}$};
            \end{axis}
        \end{tikzpicture} \\
        \begin{tikzpicture}
            \begin{axis}
    ...
    \begin{tikzpicture}
        ...
        [
            ...
            \addplot[
                ...
                domain=0.12:0.4,
                samples=200,
            ] {0.6*x^(4)} node[pos=0.5, anchor=north east] {$h^{4}$};
        \end{axis}
    \end{tikzpicture}
    \begin{tikzpicture}
        \begin{axis}
            ...
            \addplot[
                ...
                domain=0.12:0.4,
                samples=200,
            ] {5*x^(4)} node[pos=0.2, anchor=south west] {$h^{4}$};
        \end{axis}
    \end{tikzpicture}
    \begin{tikzpicture}
        \begin{axis}[
