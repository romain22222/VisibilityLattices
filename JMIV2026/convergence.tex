...
\begin{theorem}
  ...
  $\DBh{h}{X}$. Let $V$ be the $k$-visible points from $p$ in
  ...
  $\xi(p)$. Then the angle $\alpha$ between $\vn$ and the last
  eigenvector $\vv_d$ of the $V$-tangent matrix $A(p)$ follows:
  \begin{align}
    \label{eq-alpha-upper-bound}
    |\alpha| \Le O\left(\sqrt{\frac{h}{\lambda_{d-1} - \lambda_d}}\right).
  \end{align}
  Furthermore $\lambda_{d-1} - \lambda_d >0$ and tends to $1/(d-1)$.
\end{theorem}
\begin{proof}
  Assume the hypotheses, we have $\TR{\vn} A(p) \vn \Le \Theta(h)$
  ...
  the result (the term in $O(\epsilon^{\frac{3}{2}})$ is negligible
  and can be integrated to the term in $O(\sqrt{\epsilon})$ by
  changing the constants.

  We know have to study the values of the eigenvalues of
  ...
  \begin{align*}
    ...
    = O(h).
  \end{align*}
  So any $q \in V$ lies in-between two planes orthogonal to $\vn$
  around $p$, lying at a distance $O(h)$, hence in $R_h$.

  In an orthonormal basis $(\vv_i)_{i=1,\ldots,d}$,
  whose last vector $\vv_d$ is equal to $\vn$, the coordinates of
  vector $\vec{pq_j}$, $q_j \in V$, follow $(O(\sqrt{h}), \ldots,
...
\begin{corollary}
  ...
  $k$-visibility normal is a multigrid convergent normal estimator,
  with convergence speed in $O(\sqrt{h})$.

  Note that in 2d and 3d, the $k$-visibility normal estimation is
  convergent for $k \ge 2$. For higher dimensions up to 8d, the
  $k$-visibility normal estimation is convergent for $k \ge 3$.
\end{corollary}
