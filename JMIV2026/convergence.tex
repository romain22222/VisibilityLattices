...
points of $\DBh{h}{X}$, whose distances lie in the interval $\lbrack
K_1 \sqrt{h}, K_2 \sqrt{h} \rbrack$, for $0< K_1 <K_2$ arbitrary
constants, and $N$ be the cardinal of $V$.  We define the
\emph{$V$-tangent matrix} as the symmetric positive definite matrix
\begin{align*}
  A(p) & := \frac{1}{N}\sum_{q \in V} \frac{\vec{pq}}{\| \vec{pq} \|}
  \otimes \frac{\vec{pq}}{\| \vec{pq} \|}.
\end{align*}
The \emph{$V$-normal $\vec{n}_V(p)$} is the last eigenvector of $A(p)$
(corresponding to the smallest eigenvalue). Note that we have not
required yet for $V$ to contain visible points from $p$.
