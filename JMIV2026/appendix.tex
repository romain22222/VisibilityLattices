...
\end{align}
It is obvious that $M$ is a real symmetric matrix, so $M$ is
diagonalizable as $V \Lambda \TR{V}$. The matrix $V$ is an orthogonal
matrix whose column vectors $\vv_1, \ldots, \vv_d$ are called {\em
  eigenvectors} of $M$, while $\Lambda$ is a diagonal matrix whose
diagonal elements $\lambda_1, \ldots, \lambda_d$ are real values
called {\em eigenvalues} of $M$. We choose to order them in decreasing
order ($\lambda_1 \Ge \ldots \Ge \lambda_d$).

From now on, we choose the orthonormal frame so that $V$ is the
canonical orthonormal basis of $\R^d$.  For a vector $\vn$, its $j$-th
component is denoted by $n_j$, and for a vector $\vu_i$, it is
denoted by $u_{i,j}$.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \Rightarrow\quad \sum_{j=1}^{d-1} \lambda_j \vn_j^2 + \lambda_d \left(1 - \sum_{j=1}^{d-1} \vn_j^2 \right) & \Le \epsilon && \text{($\|\vn\|^2 = 1$)}\\
  ...
  \begin{align*}
    ...
    \Rightarrow~~ \lambda_d + (\lambda_{d-1} - \lambda_d)(1-n_d^2) & \Le
    \epsilon && \text{($\|\vn\|^2=1$)} \\
...
\end{proof}
Let $\phi$ be the angle between $\vn$ and $\vv_d$. The following corollary is then immediate since $n_d^2=(\vn \cdot \vv_d)^2=\cos^2\phi$:
\begin{corollary}
  \label{cor2}
  If $\TR{\vn} M \vn \Le \epsilon$, we have $|\phi| \Le
