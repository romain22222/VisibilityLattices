...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Illustration of the \replaced[author=ROM]{1st}{first} part of the proof of \RefTheorem{bound-pq-dot-n}.}
    \label{fig:fig-pq-dot-n-upper-bound}
\end{figure}

For the line $(pq)$ to be as most orthogonal as possible to the smooth boundary normal,
...
\begin{proof}
    ...
    $x=\rho-\sqrt{\rho^2-\frac{l^2}{4}}$. To conclude, \replaced[author=ROM]{one}{we} can check
    easily that $x \Le \frac{l^2}{4\rho}$ if $l \Le
    2\rho$.
\end{proof}

\begin{theorem}\label{thm-distance-pq-to-dX}
    Let $p,q$ be two pointels of $\DBh{h}{X}$,\replaced[author=ROM]{ $\alpha$ { let $\alpha$ be} some positive
...
\end{theorem}
\begin{proof}
    Let $a=\xi(p)$ and $b=\xi(q)$ be the (unique) projections of \replaced[author=ROM]{$a$ and
    $b$}{$p$ and
    $q$} onto $\partial X$. Then, \replaced[author=ROM]{by}{using} triangular inequality,
    \begin{align*}
