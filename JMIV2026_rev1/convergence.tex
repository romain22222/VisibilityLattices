...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Illustration of the \replaced[id=ROM]{first}{1st} part of the proof of \RefTheorem{bound-pq-dot-n}.}
    \label{fig:fig-pq-dot-n-upper-bound}
\end{figure}

\replaced[id=ROM]{For the line $(pq)$ to be as most orthogonal as possible to the smooth boundary normal,}
{In order that the line $(pq)$ be at most orthogonal to the normal of the smooth boundary,}
points $p$ and $q$ should not be too close
...
\begin{proof}
    ...
    intersection with the two open balls of radius $\rho$ tangent to $a$
    ...
    $x=\rho-\sqrt{\rho^2-\frac{l^2}{4}}$. To conclude, \replaced[id=ROM]{we}{one} can check
    easily that $x \Le \frac{l^2}{4\rho}$ if $l \Le
    2\rho$.
\end{proof}

\begin{theorem}\label{thm-distance-pq-to-dX}
    Let $p,q$ be two pointels of $\DBh{h}{X}$,\replaced[id=ROM]{ let $\alpha$ be}{ $\alpha$} some positive
    constant, and $h$ a small enough gridstep. If $\| \vec{pq}\|\Le
    \sqrt{\alpha h} - \sqrt{d}h$, then the segment $\lbrack p q
    \rbrack$ stays close to $\CBh{h}{X}$ and more precisely:
    \begin{align*}
        \max_{r \in \lbrack p, q \rbrack} d( r, \CBh{h}{X}) \Le (\sqrt{d}+\frac{\alpha}{4\rho})h.
    \end{align*}
\end{theorem}
\begin{proof}
    Let $a=\xi(p)$ and $b=\xi(q)$ \added[id=ROM]{be} the (unique) projections of \replaced[id=ROM]{$p$ and
    $q$}{$a$ and
    $b$} onto $\partial X$. Then, \replaced[id=ROM]{using}{by} triangular inequality,
    \begin{align*}
    ...
    For the second term, if $h$ is small enough, $\|\vec{pq}\|$ hence
    $\|\vec{ab}\|$ tend to zero so are smaller than $2\rho$, and
...
\begin{proof}
    ...
    is that
    \begin{align*}
        \max_{r \in \lbrack p q \rbrack} d(r,\DBh{h}{X}) < kh.
    \end{align*}
    Now let us fix $\beta:=\sqrt{\alpha}-\sqrt{dh}$. From $h <
    \frac{\alpha}{d}$, we have $\beta > 0$. If $\|\vec{pq}\| < \beta
    \sqrt{h}$, then $\|\vec{pq}\| < \alpha \sqrt{h} - \sqrt{d}h$.
    \RefTheorem{thm-distance-pq-to-dX} then implies
    \begin{align*}
        \max_{r \in \lbrack p, q \rbrack} d( r, \CBh{h}{X}) & < (\sqrt{d}+\frac{\alpha}{\
            4\rho})h \\
        & < \frac{4\rho\sqrt{d}+4\rho(k-\sqrt{d})}{4\rho}h\\
        & = kh,
    \end{align*}
...
\end{remark}
\begin{remark}
    In practice, we know only approximately the reach of the shape
    under study, so choosing constants $K$ and $\beta$ is not handy.
    Instead we modify the weights of visible points according to their
    distance to the point of evaluation. The visibility normal with
    the Ring kernel kills the contributions of nearby points and of points
    that are far away from the average visibility distance. This
    corresponds to a smooth variant of keeping only points at distance
    in $\Theta(\sqrt{h})$.
    %% Instead, we sort the $V_k$-visible points from $p$ according to
    %% their distance to $p$, and we simply keep the half of the points
    %% whose distance are closest to the average distance. Since we know
    %% that the average distance to $p$ is greater than some
    %% $\Omega(\sqrt{h})$ (\RefTheorem{thm-k-visibility-distance}),
    %% keeping this half is a good approximation of the targeted
    %% hypotheses for convergence. Furthermore, it avoids the necessity
