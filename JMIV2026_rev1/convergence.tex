...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Illustration of the \replaced[id=ROM]{1st}{first} part of the proof of \RefTheorem{bound-pq-dot-n}.}
...
\begin{proof}
    ...
    $x=\rho-\sqrt{\rho^2-\frac{l^2}{4}}$. To conclude, \replaced[id=ROM]{one}{we} can check
    easily that $x \Le \frac{l^2}{4\rho}$ if $l \Le
    2\rho$.
\end{proof}

\begin{theorem}\label{thm-distance-pq-to-dX}
    Let $p,q$ be two pointels of $\DBh{h}{X}$,\replaced[id=ROM]{ $\alpha$}{ let $\alpha$ be} some positive
...
\end{theorem}
\begin{proof}
    Let $a=\xi(p)$ and $b=\xi(q)$ be the (unique) projections of \replaced[id=ROM]{$a$ and
    $b$}{$p$ and
    $q$} onto $\partial X$. Then, \replaced[id=ROM]{by}{using} triangular inequality,
    \begin{align*}
