...
\added[id=ROM]{Beyond this specific application, we note that our algorithm for
computing visibility is in fact much more general. The same algorithm
could be used to recognize if any pattern of lattice points is present
on a shape. It is similar to an erosion algorithm where the
structuring element is the chosen pattern, and could also be used for
morphological operations~\cite{soille1999morphological}.}
...
algorithm, considering the primitive vectors as a hierarchy. \replaced[id=ROM]{We}{Building on this generality, we would like to explore new
