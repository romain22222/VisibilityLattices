...
In practice, we chose to test \replaced[id=ROM]{3}{three} different kernels. First, the Uniform
...
data more than further ones, arguing closer data \replaced[id=ROM]{represents better}{better represents} the
information than further \replaced[id=ROM]{one}{data}. Finally, the Ring kernel ($\Kernel{x} =
...
\begin{figure*}
    ...
    normal estimator \replaced[id=ROM]{are computed}{is} based on the mean visibility distance}
...
both failing to \replaced[id=ROM]{accomodate the saillency}{capture salient features}. It however doesn't seem
...
\begin{figure*}
    ...
    ``Integral Invariant normals`` are computed using a radius of
        ...
        normal estimator \replaced[id=ROM]{are computed using}{uses} a deviation of $\sigma=4$, and so a
...
staying \replaced[id=ROM]{meaningfull}{meaningful} in digitization of smooth parts, we compare
...
the \replaced[id=ROM]{saliencies}{salient features (digits' cavities)}.
