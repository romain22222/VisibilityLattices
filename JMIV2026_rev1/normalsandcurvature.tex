...
of our normals computed with the \replaced[id=ROM]{Gaussian}{gaussian} kernel compared
to normals computed with the integral invariant (II) estimator~\cite{Lachaud:2017-lnm}
\replaced[id=ROM]{,}{and} the PCA estimator~\cite{mitra:2003-acm}\added[id=ROM]{and the Adafit network~\cite{zhu:2021-iccv}}.
...
\begin{figure*}
    ...
    ``Integral Invariant normals`` are computed using a radius of
        $r=4.5$ \added[id=ROM]{above the line and $r=9$ below}, PCA with a discrete radius of 4 \added[id=ROM]{above the line and 8 below} (empirical)\added[id=ROM]{, Adafit uses $k=25$ neighbors}, while our
        normal estimator \replaced[id=ROM]{uses}{are computed using} a deviation of $\sigma=4$, and so a
