...
In practice, we chose to test \replaced[id=ROM]{three}{3} different kernels. First, the Uniform
...
data more than further ones, arguing closer data \replaced[id=ROM]{better represents}{represents better} the
information than further \replaced[id=ROM]{data}{one}. Finally, the Ring kernel ($\Kernel{x} =
...
that the Uniform kernel makes more errors than the other two,
...
thick the visibility cone is, since convergence is proven \replaced[id=ROM]{for $k \geq 2$}{only
starting from $k=2$}. However\added[id=ROM]{,} we did not see a significant influence on
...
\begin{figure*}
    ...
    \caption{Normal error on the GluedEllipsoid shape for different methods and different gridsteps.
    ...
    normal estimator \replaced[id=ROM]{is}{are computed} based on the mean visibility distance}
    \label{fig:errors-normals-showcase-ellipsoids}
\end{figure*}

\paragraph{Experimental validation.}

Figure~\ref{fig:normals-estimation} shows an example of our normals computed
with the \replaced[id=ROM]{Gaussian}{gaussian} kernel compared to normals
computed with the integral invariant (II) estimator~\cite{Lachaud:2017-lnm}
\replaced[id=ROM]{,}{and} the PCA estimator~\cite{mitra:2003-acm}\added[id=ROM]{
and the Adafit network~\cite{zhu:2021-iccv}. We can observe that
II and PCA normals exhibit a similar trade-off: with a small radius, they pick up noise
and digitization artifacts, but keep sharp features. With a larger radius, they
are more stable on smooth parts, but fail to capture sharp features, as they smooth them out.
Adafit normals are more stable overall, but tend to be noisy near sharp features.
Furthermore, its computational cost is significantly higher: it takes about $48$ min for the dragon shape alone, while
the other three methods combined (the 2 different radii for each method and the visibility computation included)
take about $30$ min. Our visibility normals are stable on smooth parts while capturing sharp features,
without any parameter tuning or dependency to $h$. The only optional parameter is the maximum discrete visibility radius,
which allows faster computation while producing visually indistinguishable results.}
...
on the continuous surface (Figure~\ref{fig:errors-normals}). We use 4 different digital smooth surfaces (ellipsoid ($3x^2+2y^2+z^2=90$),
...
we ignore the points that are closer than $4h$ to the singularities for Cubesphere and $8h$ for the GluedEllipsoid (see \added[id=ROM]{Figure}~\ref{fig:singularities-showcase}).
...
both failing to \replaced[id=ROM]{capture salient features}{accomodate the saillency}. It however doesn't seem
to have a better kernel between the ring and the \replaced[id=ROM]{Gaussian}{gaussian} kernel, sometimes one is better than the other, but
it really depends on what kind of shapes we are facing with. We can also notice that the singularity zone on the GluedEllipsoid
might be a bit too small for smaller gridsteps, since the error shown in \added[id=ROM]{Figure}~\ref{fig:errors-normals-showcase-ellipsoids} seems to accumulate around
...
\begin{figure}
    % 2 pictures side by side, with a name under each, singularities in red
    \centering
    \begin{tabular}{c c}
        \includegraphics[width=0.4\columnwidth]{pictures/singularities/cubesphereS} &
        \includegraphics[width=0.4\columnwidth]{pictures/singularities/double_ellipsoidS} \\
        Cubesphere & GluedEllipsoid \\
    \end{tabular}
    \caption{Singularities (in red) of the piecewise smooth shapes used to
    compute the errors in Figure~\ref{fig:errors-normals}.
    Both shapes are digitized at gridstep $h=0.05$.}
    \label{fig:singularities-showcase}
\end{figure}

\newcommand{\formatImageFigNormalesShow}[1]{%
    \includegraphics[width=0.19\textwidth]{./pictures/figNormalesDisplayV2/#1}%
}

\newcommand{\formatImageFigNormalesShowParams}[3]{%
    \begin{tikzpicture}
        \node[inner sep=0] (img)
            {\includegraphics[width=0.19\textwidth]{./pictures/figNormalesDisplayV2/#1}};
...
\begin{figure*}
    ...
    \begin{tabular}{|c||c|c|c|c|}
        \hline
        Shape & II & PCA & Adafit & Ours \\
        \hline
        \nameDisplayFigNormal{Cps} &
        \formatImageFigNormalesShowParams{cps-II-combined}{$r=4.5$}{$r=9$} &
        \formatImageFigNormalesShowParams{cps-PCA-combined}{$r=4$}{$r=8$} &
        \formatImageFigNormalesShow{cps-Adafit} &
        \formatImageFigNormalesShow{cps-VN} \\
        \hline
        \nameDisplayFigNormal{Dragon} &
        \formatImageFigNormalesShowParams{dragon-II-combined}{$r=4.5$}{$r=9$} &
        \formatImageFigNormalesShowParams{dragon-PCA-combined}{$r=4$}{$r=8$} &
        \formatImageFigNormalesShow{dragon-Adafit} &
        \formatImageFigNormalesShow{dragon-VN} \\
        \hline
        \nameDisplayFigNormal{Filigree} &
        \formatImageFigNormalesShowParams{filigree-II-combined}{$r=4.5$}{$r=9$} &
        \formatImageFigNormalesShowParams{filigree-PCA-combined}{$r=4$}{$r=8$} &
        \formatImageFigNormalesShow{filigree-Adafit} &
        \formatImageFigNormalesShow{filigree-VN} \\
        \hline
        \raisebox{9mm}{Tie Fighter} &
        \formatImageFigNormalesShowParams{tie-II-combined}{$r=4.5$}{$r=9$} &
        \formatImageFigNormalesShowParams{tie-PCA-combined}{$r=4$}{$r=8$} &
        \formatImageFigNormalesShow{tie-Adafit} &
        \formatImageFigNormalesShow{tie-VN} \\
        \hline
    \end{tabular}
    \caption{Examples of normals computed on several shapes
    as a color map. First set displays II normals, second set
    uses the PCA algorithm\added[id=ROM]{, third set uses Adafit normals} and last set uses our visibility algorithm
    ...
    ``Integral Invariant normals`` are computed using a radius of
        $r=4.5$ \added[id=ROM]{above the line and $r=9$ below}, PCA with a discrete radius of 4
        \added[id=ROM]{above the line and 8 below} (empirical)\added[id=ROM]{, Adafit uses $k=128$  neighbors},
        while our normal estimator \replaced[id=ROM]{uses capped visibility at $r=30$, with $\sigma(p)=\frac{1}{2} d_v(p)$}{are computed using a deviation of $\sigma=4$, and so a
...
staying \replaced[id=ROM]{meaningful}{meaningfull} in digitization of smooth parts, we compare
...
the \replaced[id=ROM]{salient features (digits' cavities)}{saliencies}.
