...
II and PCA normals have similar behavior : with a small radius, they pick up noise
and digitization artifacts, but keeps sharp features. With a bigger radius, they
are more stable on smooth parts, but fail to capture sharp features by smoothing
them. Adafit normals are more stable overall, but tend to be noisy on the sharp
features. Moreover, it takes a lot of time to compute them (~XXXm for cps, while
the other 3 combined are about XXXm), since it is a neural network that needs to
be evaluated for each point patch. Our visibility normals are stable on smooth
parts, while capturing the sharp features, without any parameter tuning. The
only optional parameter is the radius of visibility, by default the local mean
visibilty, which allows faster computation with results visually the same.}
