...
take about $1$ min. Our visibility normals are stable on smooth parts while capturing sharp features,
without any parameter tuning. The only optional parameter is the maximum visibility radius,
which defaults to the local mean visibility distance and allows faster computation while
producing visually indistinguishable results.}
...
\begin{figure*}
    ...
    ``Integral Invariant normals`` are computed using a radius of
        ...
        while our normal estimator \replaced[id=ROM]{uses capped visibility at $r=30$}{are computed using a deviation of $\sigma=4$, and so a
