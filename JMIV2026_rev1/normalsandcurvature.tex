...
Furthermore, its computational cost is significantly higher (~XXXm for the cps shape alone, while
the other three methods combined take about XXXm), since it is a neural network that must
be evaluated independently for each local patch.
Our visibility normals are stable on smooth
parts while capturing sharp features, without any parameter tuning. The
only optional parameter is the maximum visibility radius, which defaults to the local mean
visibility distance and allows faster computation while producing visually indistinguishable results.}
