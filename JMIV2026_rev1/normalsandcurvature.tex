...
In practice, we chose to test \replaced[author=ROM]{3}{three} different kernels. First, the Uniform
...
data more than further ones, arguing closer data \replaced[author=ROM]{represents better}{better represents} the
information than further \replaced[author=ROM]{one}{data}. Finally, the Ring kernel ($\Kernel{x} =
...
\begin{figure*}
    ...
    normal estimator \replaced[author=ROM]{are computed}{is} based on the mean visibility distance}
...
of our normals computed with the \replaced[author=ROM]{gaussian}{Gaussian} kernel compared
...
both failing to \replaced[author=ROM]{accomodate the saillency}{capture salient features}. It however doesn't seem
to have a better kernel between the ring and the \replaced[author=ROM]{gaussian}{Gaussian} kernel, sometimes one is better than the other, but
...
\begin{figure*}
    ...
    ``Integral Invariant normals`` are computed using a radius of
        ...
        normal estimator \replaced[author=ROM]{are computed using}{uses} a deviation of $\sigma=4$, and so a
...
staying \replaced[author=ROM]{meaningfull}{meaningful} in digitization of smooth parts, we compare
...
the \replaced[author=ROM]{saliencies}{salient features (digits' cavities)}.
