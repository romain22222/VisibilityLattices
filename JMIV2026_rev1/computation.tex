...
\added[id=ROM]{%
Figure~\ref{fig:visibility-algorithm-evolution} illustrates the
...
The bottom part details the core computation, which processes each
projected row (shift) $i=1,\ldots,5$ in sequence. For each row~$i$:
\begin{enumerate}
  \item $V_i$ (green interval) is the entry of the vector lattice map
    $M_\vv$ at row~$i$.
  \item $F_i$ (gray intervals) is the entry of the figure lattice map
    $\Omega$ at the corresponding shifted row $S + P.\mathrm{shift}$.
  \item $T_i$ (violet intervals) is the set of all translations~$t$
    such that $V_i + t \subseteq F_i$, i.e., all positions where the
    green interval fits entirely inside the gray intervals.
  \item $R_i$ (red intervals) is the running intersection: $R_1 = T_1$
    and $R_i = T_i \cap R_{i-1}$ for $i \geq 2$.  The black dots on
    $R_i$ mark the integer positions within these intervals.
\end{enumerate}
When a row of the vector lattice map is offset along the auxiliary axis
...
After all rows have been processed, the final result $R_5$ gives all
starting positions~$t$ along the auxiliary axis where the entire
translated vector star $\Star{[\mathbf{0},\vv]}+t$ fits inside
...
stopped early as soon as the running intersection $R_i$ becomes empty,
...
In general, the algorithm reduces to determining all possible
...
to consider all possible inclusions by translation of one interval
into another sequence of intervals), and then intersecting
progressively the results. Some results of visible points are
displayed on Figure~\ref{fig:visibility-results} on 3D shapes.
...
We also tried this measurement on an GluedEllipsoid with $\Starop_1$ and $\Starop_2$
